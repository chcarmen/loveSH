...
function showHelp() {
	...
	$("#inst_main_body").html("<div id = 'instructions_head'>玩法说明</div><br><p>该游戏目标为阻止字块超出灰色六边形区域。</p><p>" + "按左、右方向键旋转中间六边形，" + "按下方向键加速字块掉落。" + " </p><p>如相邻字块组成上海相关的词组（不限顺序），那么这些字块将自动消除、并得分。</p>");
	if (gameState == 1) {
		pause();
	}
